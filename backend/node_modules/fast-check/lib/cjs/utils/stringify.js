...
function getSymbolDescription(s) {
    ...
    const m = findSymbolNameRegex.exec((0, globals_js_1.String)(s));
...
function stringifyNumber(numValue) {
    switch (numValue) {
        ...
        default:
            return numValue === numValue ? (0, globals_js_1.String)(numValue) : 'Number.NaN';
    }
}
...
function stringifyInternal(value, previousValues, getAsyncContent) {
    ...
    if (typeof value === 'object') {
        if ((0, globals_js_1.safeIndexOf)(previousValues, value) !== -1) {
            return '[cyclic]';
        }
    }
    ...
    switch ((0, globals_js_1.safeToString)(value)) {
        case '[object Array]': {
            const arr = value;
            if (arr.length >= 50 && isSparseArray(arr)) {
                ...
                for (const index in arr) {
                    if (!safeNumberIsNaN(Number(index)))
                        (0, globals_js_1.safePush)(assignments, `${index}:${stringifyInternal(arr[index], currentValues, getAsyncContent)}`);
                }
                return assignments.length !== 0
                    ? `Object.assign(Array(${arr.length}),{${(0, globals_js_1.safeJoin)(assignments, ',')}})`
                    : `Array(${arr.length})`;
            }
            const stringifiedArray = (0, globals_js_1.safeJoin)((0, globals_js_1.safeMap)(arr, (v) => stringifyInternal(v, currentValues, getAsyncContent)), ',');
        ...
        case '[object Date]': {
            const d = value;
            return safeNumberIsNaN((0, globals_js_1.safeGetTime)(d)) ? `new Date(NaN)` : `new Date(${safeJsonStringify((0, globals_js_1.safeToISOString)(d))})`;
        }
        case '[object Map]':
        ...
        case '[object Object]': {
            ...
            const stringifiedProperties = [
                ...
                ...(0, globals_js_1.safeMap)(safeObjectKeys(value), mapper),
                ...(0, globals_js_1.safeMap)((0, globals_js_1.safeFilter)(safeObjectGetOwnPropertySymbols(value), (s) => {
                    const descriptor = safeObjectGetOwnPropertyDescriptor(value, s);
                    return descriptor && descriptor.enumerable;
                }), mapper),
            ];
            return '{' + (0, globals_js_1.safeJoin)(stringifiedProperties, ',') + '}';
        }
        case '[object Set]':
        ...
        case '[object Symbol]': {
            const s = value;
            if (globals_js_1.Symbol.keyFor(s) !== undefined) {
                return `Symbol.for(${safeJsonStringify(globals_js_1.Symbol.keyFor(s))})`;
            }
            const desc = getSymbolDescription(s);
            if (desc === null) {
                return 'Symbol()';
            }
            const knownSymbol = desc.startsWith('Symbol.') && globals_js_1.Symbol[desc.substring(7)];
    ...
    try {
        ...
        return (0, globals_js_1.safeToString)(value);
    }
}
...
function possiblyAsyncStringify(value) {
    const stillPendingMarker = (0, globals_js_1.Symbol)();
    const pendingPromisesForCache = [];
    const cache = new globals_js_1.Map();
