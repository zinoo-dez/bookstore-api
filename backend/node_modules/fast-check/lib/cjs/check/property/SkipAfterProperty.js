...
function interruptAfter(timeMs, setTimeoutSafe, clearTimeoutSafe) {
    ...
    const promise = new Promise((resolve) => {
        timeoutHandle = setTimeoutSafe(() => {
            const preconditionFailure = new PreconditionFailure_js_1.PreconditionFailure(true);
...
class SkipAfterProperty {
    ...
    run(v) {
        ...
        if (remainingTime <= 0) {
            const preconditionFailure = new PreconditionFailure_js_1.PreconditionFailure(this.interruptExecution);
            if (this.isAsync()) {
