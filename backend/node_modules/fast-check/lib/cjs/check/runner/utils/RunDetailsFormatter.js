...
const VerbosityLevel_js_1 = require("../configuration/VerbosityLevel.js");
const ExecutionStatus_js_1 = require("../reporter/ExecutionStatus.js");
...
function formatExecutionSummary(executionTrees, stringifyOne) {
    ...
    while (remainingTreesAndDepth.length !== 0) {
        ...
        const statusIcon = currentTree.status === ExecutionStatus_js_1.ExecutionStatus.Success
            ? '\x1b[32m\u221A\x1b[0m'
            : currentTree.status === ExecutionStatus_js_1.ExecutionStatus.Failure
                ? '\x1b[31m\xD7\x1b[0m'
                : '\x1b[33m!\x1b[0m';
...
function preFormatTooManySkipped(out, stringifyOne) {
    ...
    if (out.verbose >= VerbosityLevel_js_1.VerbosityLevel.VeryVerbose) {
        details = formatExecutionSummary(out.executionSummary, stringifyOne);
    }
    else {
        (0, globals_js_1.safePush)(hints, 'Enable verbose mode at level VeryVerbose in order to check all generated values and their associated status');
    }
    return { message, details, hints };
}
function prettyError(errorInstance) {
    if (errorInstance instanceof globals_js_1.Error && errorInstance.stack !== undefined) {
        return errorInstance.stack;
    }
    try {
        return (0, globals_js_1.String)(errorInstance);
    }
    catch (_err) {
    }
    if (errorInstance instanceof globals_js_1.Error) {
        try {
            return (0, globals_js_1.safeErrorToString)(errorInstance);
        }
        catch (_err) {
        }
    }
    if (errorInstance !== null && typeof errorInstance === 'object') {
        try {
            return (0, globals_js_1.safeToString)(errorInstance);
        }
        catch (_err) {
        }
    }
...
function preFormatFailure(out, stringifyOne) {
    ...
    const messageErrorPart = includeErrorInReport
        ? `\nGot ${(0, globals_js_1.safeReplace)(prettyError(out.errorInstance), /^Error: /, 'error: ')}`
        : '';
    ...
    else if (out.verbose === VerbosityLevel_js_1.VerbosityLevel.Verbose) {
        details = formatFailures(out.failures, stringifyOne);
    }
    else {
        (0, globals_js_1.safePush)(hints, 'Enable verbose mode in order to have the list of all failing values encountered during the run');
    }
    return { message, details, hints };
}
...
function defaultReportMessage(out) {
    return defaultReportMessageInternal(out, stringify_js_1.stringify);
}
async function asyncDefaultReportMessage(out) {
    const pendingStringifieds = [];
    function stringifyOne(value) {
        const stringified = (0, stringify_js_1.possiblyAsyncStringify)(value);
    ...
    const registeredValues = new globals_js_1.Map(await Promise.all(pendingStringifieds));
    function stringifySecond(value) {
        const asyncStringifiedIfRegistered = (0, globals_js_1.safeMapGet)(registeredValues, value);
...
function buildError(errorMessage, out) {
    if (out.runConfiguration.includeErrorInReport) {
        throw new globals_js_1.Error(errorMessage);
    }
    const ErrorWithCause = globals_js_1.Error;
