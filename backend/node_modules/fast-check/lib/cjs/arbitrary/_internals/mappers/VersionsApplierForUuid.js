...
function buildVersionsAppliersForUuid(versions) {
    ...
    function versionsApplierMapper(value) {
        return mapping[value[0]] + (0, globals_js_1.safeSubstring)(value, 1);
    }
    function versionsApplierUnmapper(value) {
        if (typeof value !== 'string') {
            throw new globals_js_1.Error('Cannot produce non-string values');
        }
        const rev = reversedMapping[value[0]];
        if (rev === undefined) {
            throw new globals_js_1.Error('Cannot produce strings not starting by the version in hexa code');
        }
        return rev + (0, globals_js_1.safeSubstring)(value, 1);
    }
