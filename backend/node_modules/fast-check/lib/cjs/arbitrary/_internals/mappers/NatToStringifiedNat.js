...
function natToStringifiedNatMapper(options) {
    ...
    switch (style) {
        case 'oct':
            return `0${(0, globals_js_1.safeNumberToString)(v, 8)}`;
        case 'hex':
            return `0x${(0, globals_js_1.safeNumberToString)(v, 16)}`;
        case 'dec':
        default:
            return `${v}`;
    }
...
function tryParseStringifiedNat(stringValue, radix) {
    ...
    if ((0, globals_js_1.safeNumberToString)(parsedNat, radix) !== stringValue) {
...
function natToStringifiedNatUnmapper(value) {
    ...
    if (value.length >= 2 && value[0] === '0') {
        if (value[1] === 'x') {
            return ['hex', tryParseStringifiedNat((0, globals_js_1.safeSubstring)(value, 2), 16)];
        }
        return ['oct', tryParseStringifiedNat((0, globals_js_1.safeSubstring)(value, 1), 8)];
    }
