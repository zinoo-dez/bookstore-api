...
function buildReverseMapping(entries) {
    const reverseMapping = { mapping: new globals_js_1.Map(), negativeZeroIndex: undefined };
    let choiceIndex = 0;
    for (let entryIdx = 0; entryIdx !== entries.length; ++entryIdx) {
        const entry = entries[entryIdx];
        for (let idxInEntry = 0; idxInEntry !== entry.num; ++idxInEntry) {
            const value = entry.build(idxInEntry);
            if (value === 0 && 1 / value === globals_js_1.Number.NEGATIVE_INFINITY) {
                reverseMapping.negativeZeroIndex = choiceIndex;
            }
            else {
                (0, globals_js_1.safeMapSet)(reverseMapping.mapping, value, choiceIndex);
            }
            ++choiceIndex;
        }
    }
...
function indexToMappedConstantUnmapperFor(entries) {
    ...
    return function indexToMappedConstantUnmapper(value) {
        ...
        const choiceIndex = safeObjectIs(value, -0)
            ...
            : (0, globals_js_1.safeMapGet)(reverseMapping.mapping, value);
        if (choiceIndex === undefined) {
            throw new globals_js_1.Error('Unknown value encountered cannot be built using this mapToConstant');
        }
        return choiceIndex;
    };
}
