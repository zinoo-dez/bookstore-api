...
function wordsToJoinedStringMapper(words) {
    return (0, globals_js_1.safeJoin)((0, globals_js_1.safeMap)(words, (w) => (w[w.length - 1] === ',' ? (0, globals_js_1.safeSubstring)(w, 0, w.length - 1) : w)), ' ');
}
function wordsToJoinedStringUnmapperFor(wordsArbitrary) {
    return function wordsToJoinedStringUnmapper(value) {
        ...
        const words = [];
        for (const candidate of (0, globals_js_1.safeSplit)(value, ' ')) {
            if (wordsArbitrary.canShrinkWithoutContext(candidate))
                (0, globals_js_1.safePush)(words, candidate);
...
function wordsToSentenceMapper(words) {
    let sentence = (0, globals_js_1.safeJoin)(words, ' ');
    if (sentence[sentence.length - 1] === ',') {
        sentence = (0, globals_js_1.safeSubstring)(sentence, 0, sentence.length - 1);
    }
    return (0, globals_js_1.safeToUpperCase)(sentence[0]) + (0, globals_js_1.safeSubstring)(sentence, 1) + '.';
}
function wordsToSentenceUnmapperFor(wordsArbitrary) {
    return function wordsToSentenceUnmapper(value) {
        ...
        if (value.length < 2 ||
            ...
            (0, globals_js_1.safeToUpperCase)((0, globals_js_1.safeToLowerCase)(value[0])) !== value[0]) {
            throw new Error('Unsupported value');
        }
        const adaptedValue = (0, globals_js_1.safeToLowerCase)(value[0]) + (0, globals_js_1.safeSubstring)(value, 1, value.length - 1);
        const words = [];
        const candidates = (0, globals_js_1.safeSplit)(adaptedValue, ' ');
...
function sentencesToParagraphMapper(sentences) {
    return (0, globals_js_1.safeJoin)(sentences, ' ');
}
function sentencesToParagraphUnmapper(value) {
    if (typeof value !== 'string') {
        throw new Error('Unsupported type');
    }
    const sentences = (0, globals_js_1.safeSplit)(value, '. ');
