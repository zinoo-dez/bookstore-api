...
exports.invertSize = invertSize;
const GlobalParameters_js_1 = require("../../../check/runner/configuration/GlobalParameters.js");
...
function relativeSizeToSize(size, defaultSize) {
    const sizeInRelative = (0, globals_js_1.safeIndexOf)(orderedRelativeSize, size);
    if (sizeInRelative === -1) {
        return size;
    }
    const defaultSizeInSize = (0, globals_js_1.safeIndexOf)(orderedSize, defaultSize);
...
function maxGeneratedLengthFromSizeForArbitrary(size, minLength, maxLength, specifiedMaxLength) {
    const { baseSize: defaultSize = exports.DefaultSize, defaultSizeToMaxWhenMaxSpecified } = (0, GlobalParameters_js_1.readConfigureGlobal)() || {};
...
function resolveSize(size) {
    const { baseSize: defaultSize = exports.DefaultSize } = (0, GlobalParameters_js_1.readConfigureGlobal)() || {};
    if (size === undefined) {
        return defaultSize;
    }
    return relativeSizeToSize(size, defaultSize);
}
function invertSize(size) {
    switch (size) {
        case 'xsmall':
            return 'xlarge';
        case 'small':
