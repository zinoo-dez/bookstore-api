...
function percentCharArbMapper(c) {
    const encoded = (0, globals_js_1.encodeURIComponent)(c);
    return c !== encoded ? encoded : `%${(0, globals_js_1.safeNumberToString)((0, globals_js_1.safeCharCodeAt)(c, 0), 16)}`;
}
...
const percentCharArb = () => (0, string_js_1.string)({ unit: 'binary', minLength: 1, maxLength: 1 }).map(percentCharArbMapper, percentCharArbUnmapper);
let lowerAlphaArbitrary = undefined;
function getOrCreateLowerAlphaArbitrary() {
    if (lowerAlphaArbitrary === undefined) {
        lowerAlphaArbitrary = (0, mapToConstant_js_1.mapToConstant)(lowerCaseMapper);
    }
...
function getOrCreateLowerAlphaNumericArbitrary(others) {
    ...
    let match = (0, globals_js_1.safeMapGet)(lowerAlphaNumericArbitraries, others);
    if (match === undefined) {
        match = (0, mapToConstant_js_1.mapToConstant)(lowerCaseMapper, numericMapper, {
            num: others.length,
            build: (v) => others[v],
        });
        (0, globals_js_1.safeMapSet)(lowerAlphaNumericArbitraries, others, match);
    }
    return match;
}
function buildAlphaNumericArbitrary(others) {
    return (0, mapToConstant_js_1.mapToConstant)(lowerCaseMapper, upperCaseMapper, numericMapper, {
        num: others.length,
        build: (v) => others[v],
    });
}
...
function getOrCreateAlphaNumericPercentArbitrary(others) {
    ...
    let match = (0, globals_js_1.safeMapGet)(alphaNumericPercentArbitraries, others);
    if (match === undefined) {
        match = (0, oneof_js_1.oneof)({ weight: 10, arbitrary: buildAlphaNumericArbitrary(others) }, { weight: 1, arbitrary: percentCharArb() });
        (0, globals_js_1.safeMapSet)(alphaNumericPercentArbitraries, others, match);
    }
    return match;
}
